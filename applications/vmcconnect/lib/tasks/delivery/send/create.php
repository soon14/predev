...
// +----------------------------------------------------------------------
class vmcconnect_tasks_delivery_send_create extends vmcconnect_tasks_base implements base_interface_task{
    ...
    //如果推送失败则执行下面的方法
    public function fail_comment ($data) {
        //如果失败，则执行失败需添加的内容
        //①添加到hook日志内部
        //②统计到fail_count数据库表内
        //③统计当前方法的错误次数是否达到发送警报的次数
        return $this->queue_exception_handling($data, $task);
    }
