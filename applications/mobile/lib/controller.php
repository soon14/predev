...
class mobile_controller extends base_controller
{
    ...
    */
    public function __construct(&$app)
    {
        ...
        foreach (vmc::servicelist('mobile.controller.construct') as $object) {
        ...
        if( base_component_request::is_wxapp() ){
            foreach($this->_response->get_headers() AS $header){
                if(strtolower($header['name']) == strtolower('Set-wxappstorage')){
                    $this->pagedata['X-WXAPPSTORAGE'][] = $header['value'];
                }
            }
            unset($this->pagedata['_MAIN_M_']);
            unset($this->pagedata['_THEME_M_']);
            $content = json_encode($this->pagedata);
            if ($fetch === true) {
                return $content;
            } else {
                echo $content;
    ...
    */
    final public function display_tmpl($tmpl, $fetch = false, $is_preview = false)
    {
        ...
        $this->_vars['is_wechat'] = base_mobiledetect::is_wechat();
        $this->_vars['is_hybirdapp'] = base_mobiledetect::is_hybirdapp();
    ...
    public function pre_display(&$content){
        ...
        if($this->_ignore_pre_display === false){
            foreach(vmc::serviceList('mobile_render_pre_display') AS $service){
    ...
    */
    public function splash($status = 'success', $url = '', $msg = null, $ajax = false, $data = null)
    {
        ...
        if ($url != '' && !$_COOKIE['vmc-hybirdapp-inpresent']) {
        ...
        $this->page('splash.html', false, 'mobile');
