...
class sale_ctl_site_sale extends b2c_frontpage
{
    ...
    public function get_status($product_id)
    {
        ...
        echo $this->fetch('site/product/status/'.$page.'.html');
    }
    ...
    public function reserve(){
        if(!$_POST['sale_id']){
            ...
            $this->pagedata['sale_id'] = $_POST['sale_id'];
        }
        $this->verify_member();
        $mdl_reserve = $this->app->model('reserve');
        $member_id = vmc::singleton('b2c_user_object')->get_member_id();
        $reserve = $mdl_reserve->getRow('*',array('goods_id'=>$_POST['goods_id'],'member_id'=>$member_id,'sale_id'=>$_POST['sale_id']));
        if(!empty($reserve)){
            $this->pagedata['reserve'] = true;
        }
        $this->pagedata['goods_id'] = $_POST['goods_id'];
        echo $this->fetch('site/product/reserve_form.html');
    }
}
