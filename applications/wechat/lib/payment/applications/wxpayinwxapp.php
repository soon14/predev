...
final class wechat_payment_applications_wxpayinwxapp extends ectools_payment_parent implements ectools_payment_interface
{
    public $name = '微信小程序支付';
    public $version = '';
    public $intro = '微信小程序支付是Tencent腾讯旗下微信平台的小程序专用支付方式。';
    ...
    public function __construct($app)
    {
        ...
        $this->callback_url = vmc::openapi_url('openapi.ectools_payment', 'getway_callback', array(
            'wechat_payment_applications_wxpayinwxapp' => 'callback',
        ));
        $this->notify_url = vmc::openapi_url('openapi.ectools_payment', 'getway_callback', array(
            'wechat_payment_applications_wxpayinwxapp' => 'notify',
        ));
    }
    /**
     * 后台配置参数设置.
    ...
    public function setting()
    {
        return array(
            ...
            'appid' => array(
                'title' => 'appid',
                'type' => 'text',
                'default'=>app::get('wechat')->getConf('wxxcx_appid')
            ),
            'appsecret' => array(
                'title' => 'appsecret',
                'type' => 'text',
                'default'=>app::get('wechat')->getConf('wxxcx_appsecret')
            ),
            'mch_id' => array(
                'title' => 'mch_id(商户号)',
                'type' => 'text',
            ...
            'apiclient_cert' => array(
                ...
                'type' => 'textarea',
                'file_url' => 'wxpayinwxapp/apiclient_cert.pem',
            ),
            'apiclient_key' => array(
                'title' => '证书密钥apiclient_key',
                'type' => 'textarea',
                'file_url' => 'wxpayinwxapp/apiclient_key.pem',
            ),
            /*个性化字段结束*/
            'pay_fee' => array(
                'title' => '交易费率 (%)' ,
    ...
    public function dopay($params, &$msg)
    {
        ...
        $post_data = array(
            ...
            'trade_type' => 'JSAPI',
            'openid' => $params['openid']?$params['openid']:($_GET['openid']?$_GET['openid']:$_POST['openid']),
        );
        ...
        if ($res_arr['return_code'] != 'SUCCESS' || $res_arr['result_code'] != 'SUCCESS') {
            $err_msg = $res_arr['return_msg'].' '.$res_arr['err_code_des'];
        ...
        $ready_params['paySign'] = $this->_get_mac($mch_key, $ready_params);
        unset($ready_params['appId']);
        echo json_encode($ready_params);
        exit;
    }
