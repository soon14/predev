...
final class wechat_payment_applications_wxpayinapp extends ectools_payment_parent implements ectools_payment_interface
{
    public $name = '微信APP支付';
    public $version = '';
    ...
    public function __construct($app)
    {
        ...
        $this->callback_url = vmc::openapi_url('openapi.ectools_payment', 'getway_callback', array(
            'wechat_payment_applications_wxpayinapp' => 'callback',
        ));
...
HTML;
    ...
    public function setting()
    {
        return array(
            ...
            'apiclient_cert' => array(
                ...
                'type' => 'textarea',
                'file_url' => 'wxpayinapp/apiclient_cert.pem',
            ),
            'apiclient_key' => array(
                'title' => '证书密钥apiclient_key',
                'type' => 'textarea',
                'file_url' => 'wxpayinapp/apiclient_key.pem',
            ),
            /*个性化字段结束*/
            'pay_fee' => array(
                'title' => '交易费率 (%)' ,
    ...
    public function dopay($params, &$msg)
    {
        ...
        if ($res_arr['return_code'] != 'SUCCESS' || $res_arr['result_code'] != 'SUCCESS') {
            ...
            $render->pagedata['err_msg'] = $err_msg;
        ...
        $ready_params = array(
            'appid' => $appid,
            'partnerid' => $mch_id,
            'prepayid' => $res_arr['prepay_id'],
            'package' => 'Sign=WXPay',
            'noncestr' => $this->getNonceStr(),
            'timestamp' => (string) time(),
        );
        $ready_params['sign'] = $this->_get_mac($mch_key, $ready_params);
        $render->pagedata['ready_params'] = $ready_params;
        echo $render->fetch('wxpay/hybirdapp.html');
        exit;
