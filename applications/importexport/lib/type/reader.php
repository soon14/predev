...
class importexport_type_reader implements \SeekableIterator, \Countable {
	...
	public function __construct($filePath, $originalFileName = false, $mimeType = false ,$fileType = false) {
		...
		$Extension = strtolower($fileType ? : pathinfo($originalFileName, PATHINFO_EXTENSION));
		if($mimeType) {
			switch ($mimeType) {
		...
		if ($this->type == self::TYPE_XLS) {
			$this->handle = new importexport_type_excel_XLS($filePath);
		...
		switch ($this->type) {
			case self::TYPE_XLSX:
				$this->handle = new importexport_type_excel_XLSX($filePath);
				break;
			case self::TYPE_CSV:
				$this->handle = new importexport_type_excel_CSV($filePath, 1);
				break;
			case self::TYPE_XLS:
    ...
    {
        if (!$this->handle) {
            throw new Exception('PHPExcel_Reader: No file opened');
        }
	...
	public function seek($position = false){
		...
		if ($Currentindex != $position) {
			...
			if ( ! $this->handle->valid()) {
                ...
                throw new Exception('PHPExcel_Reader: position ' . $position . ' not found');
			}
		}
