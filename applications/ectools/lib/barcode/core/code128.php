...
class ectools_barcode_core_code128
{
    /*
     * @var data - to be set
     */
     ...
     * @return html
     */
    public function draw($text, $height = 60)
    {
        $this->setData($text);
        $this->setSubType(1);
        $this->resolveSubtype();
        ...
        foreach ($charAry as $k => $char) {
            ...
            foreach (str_split((string) $code) as $bit) {
                $loop_color = ($bit == '1' ? '#000' : '#fff');
                $code_bar .= "<div style='position:absolute;width:".$pxPerBar.'px;top:0;left:'.$currentX.'px;height:100%;background-color:'.$loop_color.";overflow:hidden;'></div>";
                $currentX += $pxPerBar;
            }
        ...
        $code_div_begin = "<div class='x-barcode' data-val='".$text."' style='width:".$currentX.'px;position:relative;height:'.$height."px;margin-bottom:18px;'>";
        $code_str = "<div style='text-align:center;font-size:12px;font-weight:bold;line-height:normal;width:100%;left:0;bottom:-18px;position:absolute;'>".$text.'</div>';
        $code_div_end = '</div>';
        $code_html = $code_div_begin.$code_bar.$code_str.$code_div_end;
