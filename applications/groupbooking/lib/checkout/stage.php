...
class groupbooking_checkout_stage
{
    ...
    public function check($params)
    {
        ...
        $activity_result['product'] = app::get('b2c')->model('products')->getRow('*',array('product_id'=>$params['product_id'],'marketable' => 'true'));
        $activity_result = array_merge($activity_result,$activity['conditions'][$params['product_id']]);
        $activity_result['order_total'] = $obj_math->number_multiple(array($quantity,$activity_result['price']));
        if($activity_result['freight'] > 0) {
            $activity_result['order_total'] = $obj_math->number_plus(array($activity_result['order_total'],$activity_result['freight']));
        }
    ...
    public function changepayment($id, $new_payappid, &$msg)
    {
        ...
        $order = $mdl_orders->getRow('pay_status,payed,pay_app,cost_freight,order_total', array('gb_id' => $id));
        ...
        if (!$mdl_orders->update(array('pay_app' => $new_payappid), array('gb_id' => $id))) {
            $msg = '支付方式修改失败.';
