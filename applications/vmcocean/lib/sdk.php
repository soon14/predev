...
define('VMCOCEAN_ANALYTICS_SDK_VERSION', '1.5.0');

class VMCOceanAnalyticsException extends \Exception {
}

// 在发送的数据格式有误时，SDK会抛出此异常，用户应当捕获并处理。
class VMCOceanAnalyticsIllegalDataException extends VMCOceanAnalyticsException {
}

// 在因为网络或者不可预知的问题导致数据无法发送时，SDK会抛出此异常，用户应当捕获并处理。
class VMCOceanAnalyticsNetworkException extends VMCOceanAnalyticsException {
}

// 当且仅当DEBUG模式中，任何网络错误、数据异常等都会抛出此异常，用户可不捕获，用于测试SDK接入正确性
class VMCOceanAnalyticsDebugException extends \Exception {
}

// 不支持 Windows，因为 Windows 版本的 PHP 都不支持 long
if (strtoupper(substr(PHP_OS, 0, 3)) == "WIN") {
    throw new VMCOceanAnalyticsException("VMCOcean Analytics PHP SDK dons't not support Windows");
}

class VMCOceanAnalytics {

    private $_consumer;
    private $_super_properties;

    /**
     * 初始化一个 VMCOceanAnalytics 的实例用于数据发送。
     *
    ...
    private function _normalize_data($data) {
        ...
        if (!isset($data['distinct_id']) or strlen($data['distinct_id']) == 0) {
            throw new VMCOceanAnalyticsIllegalDataException("property [distinct_id] must not be empty");
        }
        if (strlen($data['distinct_id']) > 255) {
            throw new VMCOceanAnalyticsIllegalDataException("the max length of [distinct_id] is 255");
        }
        ...
        if ($ts_num < 10 || $ts_num > 13) {
            throw new VMCOceanAnalyticsIllegalDataException("property [time] must be a timestamp in microseconds");
        }
        ...
        if (isset($data['event']) && !preg_match($name_pattern, $data['event'])) {
            throw new VMCOceanAnalyticsIllegalDataException("event name must be a valid variable name. [name='${data['event']}']");
        }
        ...
        if (isset($data['properties']) && is_array($data['properties'])) {
            foreach ($data['properties'] as $key => $value) {
                if (!is_string($key)) {
                    throw new VMCOceanAnalyticsIllegalDataException("property key must be a str. [key=$key]");
                }
                if (strlen($data['distinct_id']) > 255) {
                    throw new VMCOceanAnalyticsIllegalDataException("the max length of property key is 256. [key=$key]");
                }

                if (!preg_match($name_pattern, $key)) {
                    throw new VMCOceanAnalyticsIllegalDataException("property key must be a valid variable name. [key='$key']]");
                }

                // 只支持简单类型或数组或DateTime类
                if (!is_scalar($value) && !is_array($value) && !$value instanceof DateTime) {
                    throw new VMCOceanAnalyticsIllegalDataException("property value must be a str/int/float/datetime/list. [key='$key' value='$value']");
                }
                ...
                if (is_string($value) && strlen($data['distinct_id']) > 8191) {
                    throw new VMCOceanAnalyticsIllegalDataException("the max length of property value is 8191. [key=$key]");
                }

                // 如果是数组，只支持 Value 是字符串格式的简单非关联数组
                if (is_array($value)) {
                    if (array_values($value) !== $value) {
                        throw new VMCOceanAnalyticsIllegalDataException("[list] property must not be associative. [key='$key']");
                    }

                    foreach ($value as $lvalue) {
                        if (!is_string($lvalue)) {
                            throw new VMCOceanAnalyticsIllegalDataException("[list] property's value must be a str. [value='$lvalue']");
                        }
                    }
                }
            }
            ...
            throw new VMCOceanAnalyticsIllegalDataException("property must be an array.");
        }
        return $data;
    }
    ...
    private function _get_lib_properties() {
        $lib_properties = array(
                '$lib' => 'php',
                '$lib_version' => VMCOCEAN_ANALYTICS_SDK_VERSION,
                '$lib_method' => 'code',
                );
    ...
    public function track_signup($distinct_id, $original_id, $properties = array()) {
        ...
        if (!$original_id or strlen($original_id) == 0) {
            throw new VMCOceanAnalyticsIllegalDataException("property [original_id] must not be empty");
        }
        if (strlen($original_id) > 255) {
            throw new VMCOceanAnalyticsIllegalDataException("the max length of [original_id] is 255");
        }
...
class DebugConsumer extends AbstractConsumer {
     ...
     * @throws VMCOceanAnalyticsDebugException
     */
    public function __construct($url_prefix, $write_data = True, $request_timeout = 1000) {
        $parsed_url = parse_url($url_prefix);
        if ($parsed_url === false) {
            throw new VMCOceanAnalyticsDebugException("Invalid server url of VMCOcean Analytics.");
        }
    ...
    public function send($msg) {
        ...
        $ret_content = $response['ret_content'];
        if ($response['ret_code'] === 200) {
            logger::alert("valid message: $msg");
        } else {
            logger::alert("invalid message: $msg");
            logger::alert("ret_code: $ret_code");
            logger::alert("ret_content: $ret_content");
        }
        if ($response['ret_code'] >= 300) {
            throw new VMCOceanAnalyticsDebugException("Unexpected response from VMCOceanAnalytics.");
        }
    }
    ...
    protected function _do_request($data) {
        ...
        if (!$http_response_header) {
            throw new VMCOceanAnalyticsDebugException(
                   "Failed to connect to VMCOceanAnalytics. [error='" + curl_error($ch) + "']");
        }
