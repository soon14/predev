...
class vmcocean_order_reship
{
    ...
    public function exec($delivery_sdf, &$msg = '')
    {
        ...
        $delivery_data = array(
            ...
            'ConsignorArea'=>explode(':',$delivery_sdf['consignor_area'])[1],
        ...
        $sa_stage->track_event($delivery_sdf['member_id'],'DeliveryReship',$delivery_data);
        foreach ($delivery_sdf['delivery_items'] as $key => $item) {
            ...
            $sa_stage->track_event($delivery_sdf['member_id'],'DeliveryReshipItem',$delivery_data_item);
        }
        return true;
    }
}
