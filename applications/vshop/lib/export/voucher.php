...
class vshop_export_voucher
{
    ...
    public function doexport($filter = array())
    {
        ...
        $voucher_items = app::get('vshop')->model('voucher_items')->getList('*',array('voucher_id'=>$voucher_id_arr));
        $exporter = new vshop_export_excel('browser','voucher-'.date('YmdHis').'.xls');
        $exporter->initialize();
