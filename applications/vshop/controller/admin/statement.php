...
class vshop_ctl_admin_statement extends desktop_controller
{
    public function index()
    {
        $this->finder('vshop_mdl_statement', array(
            'title' => ('财务结算单'),
            'use_buildin_recycle' => $this ->has_permission('vshop_statement_delete'),
            //'use_buildin_export'=>true,
            'use_buildin_set_tag' => $this ->has_permission('vshop_statement_tag'),
    ...
    public function edit($statement_id)
    {
        ...
        $this->pagedata['vshop'] = $mdl_vshop->dump($statement['shop_id']);
        $this->page('admin/statement/edit.html');
    }
    public function update($statement_id)
    {
        $this->begin('index.php?app=vshop&ctl=admin_statement&act=index');
        ...
        if ($statement_data['status'] == 'succ') {
            foreach ($statement['statement_index'] as $index) {
                ...
                if (!$mdl_voucher->save($update_voucher)) {
                    ...
                    foreach (vmc::servicelist('vshop.voucher.update') as $service) {
        ...
        if ($mdl_statement->save($statement_data)) {
            foreach (vmc::servicelist('vshop.statement.update') as $service) {
    ...
    public function create()
    {
        ...
        $vshop_id_arr = array_keys(utils::array_change_key($voucher_arr, 'shop_id'));
        if (!empty($vshop_id_arr[1])) {
            $this->pagedata['multiple_vshop'] = 'true';
        }
    ...
    public function docreate()
    {
        ...
        $statement_data['shop_id'] = $voucher_arr[0]['shop_id'];
        $statement_data['shop_name'] = $voucher_arr[0]['shop_name'];
        ...
        if (!$mdl_statement->save($statement_data)) {
            ...
            foreach (vmc::servicelist('vshop.statement.create') as $service) {
                if (!$service->exec($statement_data, $msg)) {
                    logger::error($msg);
    ...
    public function docreate_multiple()
    {
        ...
        $voucher_arr_vshop_group = utils::array_change_key($voucher_arr, 'vshop_id', true);
        $mdl_statement = $this->app->model('statement');
        foreach ($voucher_arr_vshop_group as $vshop_id => $voucher_arr) {
