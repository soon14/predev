...
// +----------------------------------------------------------------------
class ubalance_ctl_mobile_balancepay extends b2c_mfrontpage
{
    ...
    public function do_payment()
    {
        ...
        if (!vmc::singleton('ubalance_account')->fund_change($log_data, $msg)) {
            ...
            if ($bill['return_url']) {
                ...
                $return_url = array('app' => 'b2c','ctl' => 'mobile_order','act' => 'detail','args' => array($bill['order_id']));
            }
    ...
    public function do_recharge($bill_id = '', $recursive)
    {
        ...
        //如果是微信支付回调，微信内支付时，支付方式是微信
        if(!$recursive && !base_mobiledetect::is_wechat() && $_POST['payapp_id'] == 'wxpay') {
            //金额判断
        ...
        if (base_mobiledetect::is_wechat() && $_POST['payapp_id'] == 'wxpay' && empty($bill_sdf['payer_account'])) {
            ...
            if (!$recursive) {
                $oauth_redirect = $this->gen_url(array(
                    ...
                    'ctl' => 'mobile_balancepay',
                    'act' => 'do_recharge',
                    'args' => array($bill_sdf['bill_id'], 'recursive'),
                    'full' => 1,
                ));
    ...
    public function recharge_result($bill_id)
    {
        ...
        $this->page('mobile/pay/result.html');
    }
}
