...
class commission_ctl_mobile_member extends b2c_ctl_mobile_member
{
    ...
    public function become(){
        $member = $this->app_current ->model('member_relation') ->getRow('*' ,array('member_id' =>$this ->member['member_id']));
        if($member['is_commission'] == '1'){
            $this ->redirect(array('app'=>'b2c','ctl' =>'mobile_member' ,'act' =>'index' ));
        }
        ...
        if($this ->_request ->is_post()){
            ...
            $redirect = $_SERVER['HTTP_REFERER'] ? $_SERVER['HTTP_REFERER'] : $this ->gen_url(array('app'=>'b2c','ctl' =>'site_member' ,'act' =>'index' ));
    ...
    public function my()
    {
        ...
        //上关系
        $member = $this->app_current->model('member_relation')->getRow("parent_id",array("member_id" => $this->member['member_id']));
        $user_obj = vmc::singleton('b2c_user_object');
        if($member['parent_id']){
            $this ->pagedata['parent'] =  $user_obj->get_member_info( $member['parent_id']);
        }
        //下级信息获取
        $children = $this->app_current->model('member_relation')->getList("member_id",array("parent_id" => $this->member['member_id']));
        ...
        $total = $this->app_current->model('orderlog')->count(array("orderlog_id" => $orderlogid));
        ...
        $orderlog_list = $this->app_current->model('orderlog')->getList("*",
            array("orderlog_id" => $orderlogid), $page['size'] * ($page['index'] - 1),
        ...
        foreach($orderlog_list as $k =>$v){
            ...
            if(!empty($children) && $v['settle_status'] == 1 && in_array($v['from_id'] ,array_keys(utils::array_change_key($children ,'member_id')))){
    ...
    public function cash()
    {
        ...
        if ($this->_request->is_post()) {
            ...
            $member_info = $this->app_current ->model('member_relation') ->getRow('*' ,array('member_id' =>$this ->member['member_id']));
    ...
    public function account()
    {
        ...
        $page = $this->get_limit(3);
        $total = $this->app_current->model('fundlog')->count(array("member_id" => $this->member['member_id'], "type" => array(2, 4, 6)));
        $params = utils::_filter_input($_GET);
        $this->pagedata['query'] = $this->_query_str($params, 0);
        $this->pagedata['pager'] = $this->get_pager($total, $page, $params, 'account');
        $fundlog = $this->app_current->model('fundlog')->getList("*",
            array("member_id" => $this->member['member_id'], "type" => array(2, 4, 6)), $page['size'] * ($page['index'] - 1),
    ...
    private function _check_is_commission(){
        ...
        if(!$member || $member['is_commission'] === '0'){
            $this ->redirect(array('app'=>'commission','ctl' =>'mobile_member' ,'act' =>'become' ));
        }
