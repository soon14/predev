...
class supplier_mdl_voucher extends dbeav_model
{
    public $has_tag = true;
    ...
    public function count_subprice($supplier_id, $from = false, $to = false,$status = false)
    {
        ...
        if($supplier_id){
            if(is_array($supplier_id)){
                $where_str .= ' AND v.supplier_id IN '."('" . implode("','", $supplier_id) . "')";
            }else{
                $where_str .= ' AND v.supplier_id = '.$supplier_id;
            }
        }
