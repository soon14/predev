...
class o2ocds_desktop_widgets_overview implements desktop_interface_widget
{
    ...
    public function get_html($from, $to)
    {
        ...
        $SQL_order_analytics = "SELECT FROM_UNIXTIME(createtime,'$date_str_format') as date_str,count(createtime) as count,sum(order_fund) as amount FROM vmc_o2ocds_orderlog  WHERE (settle_status <>'2')  AND " . $order_where_ft . ' GROUP BY date_str ORDER BY date_str';
